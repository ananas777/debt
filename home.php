...
$regexp = "/[\n]/u";
$sentences = preg_split($regexp, $text);//разбивает строку по регулрному выражению
...
for ($i=0; $i < count($sentences); $i++) { 
	...
	$sentences[$i] .= str_repeat("1", ($maxStrLen - mb_strlen($sentences[$i])-1));//возвращение повторяющийся строки
}

//разделение каждой строки на массив по буквам
for ($i=0; $i < count($sentences); $i++) { 
	$sentences[$i] = preg_split("//u", $sentences[$i], -1, PREG_SPLIT_NO_EMPTY);//функция preg_split() вернет только непустые подстроки
}
