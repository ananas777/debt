...
function makeFirstLetterUppercase($text) {
  return mb_strtoupper(mb_substr($text, 0, 1)) . mb_substr($text, 1, NULL);// mb_strtoupper приведения строки к верхнему регистру, mb_substr возвращает часть строки
}
...
function smallNumberToText($number, $isFemale) {
    ...
    if ($lastDigit != 0) {
      ...
      return implode($text, ' ');//объединяет элементы массива в строку 
  }
